...
import ShipToLocTable from '../CustomerTabs/CustomerInformation/ShipToLocTable';
...
describe('EditUser', () => {
  ...
  beforeEach(async () => {
    ...
    wrapper = mount(ShallowMock(<ShipToLocTable />, store));
  ...
  it('ShipToLocTable are there', () => {
    expect('ShipToLocTable').toBeTruthy();
  });
  ...
  it('onClick on  ACtionComponent', () => {
    const button = wrapper.find('ForwardRef(PaginationItem)');

    button.at(0).props().onClick();
  });

  it('onClick on  ACtionComponent', () => {
    const button = wrapper.find('WithStyles(ForwardRef(ButtonBase))');

    button.at(0).props().onClick();
  });

  it('onClick on  ACtionComponent', () => {
    const button = wrapper.find('ForwardRef(Checkbox)');

    button.at(0).props().onChange();
  });

  it('onClick on  ACtionComponent', () => {
    const button = wrapper.find('ForwardRef(SwitchBase)');
